...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < units.Length; i++)
        {
            ...
            float angle = i * 360f / currentUnitCount;

            // 리더 앞 기준 회전 방향
            Vector3 direction = Quaternion.AngleAxis(angle, leader.up) * leader.forward;
            units[i].position = leader.position + (direction * formationRadius);



            // TODO
            // 그냥 direction을 leader.rotation에 곱해도 됌
            Vector3 look = leader.position - units[i].position;

            units[i].rotation = Quaternion.LookRotation(-look, Vector3.up);
        }
