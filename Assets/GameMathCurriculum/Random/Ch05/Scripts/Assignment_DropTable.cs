...
public class Assignment_DropTable : MonoBehaviour
{
    ...
    [SerializeField] private DropItem[] items = new DropItem[]
    {
        ...
        new DropItem { name = "에픽(보라)", weight = 5f, color = new Color(1.0f, 0.2f, 1.0f) },
        new DropItem { name = "전설(노랑)", weight = 1f, color = new Color(1.0f, 0.92f, 0.016f) },
        new DropItem { name = "신화(빨강)", weight = 0.1f, color = new Color(0.8f, 0.0f, 0.0f) },
    };
    ...
    private void PerformDrop()
    {
        ...
        foreach (var item in items)
        {
            ...
            if (probability > randomWeight)
            {
                dropHistory.Add(index);
