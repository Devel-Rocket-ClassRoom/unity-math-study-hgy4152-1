...
public class Assignment_GachaSimulator : MonoBehaviour
{
    ...
    private void ExecutePull()
    {
        ...
        {
            currentEffectiveRate += 1f / (hardPity - softPityStart);
        }


        if (r <= currentEffectiveRate)
        {
            totalSSRs++;

            HistoryAdd(currentPityCount);
        ...
        // 안뽑히면
        HistoryAdd(pull: false);


    }

    private void HistoryAdd(int count = 0, bool pull = true)
    {

        if(count > 0)
            ssrPityList.Insert(0, count);
